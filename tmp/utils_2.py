...
def snp_2_gene(assoc_f='results/gwas/arabi.qassoc', out_f='results/gwas/arabi.gwas.snp.p.txt', sg_f='data/arabi.20kb.sg', g_f='data/arabi.gene'):
	tb_1 = pd.read_csv(assoc_f, sep='\s*', usecols=['SNP','P'])
	tb_1['P'] = -np.log(tb_1['P'].values)
	tb_2 = pd.read_csv(sg_f, header=None, sep=' ')
	tb_2.columns = ['SNP', 'gene']
	tb_3 = pd.read_csv(g_f, header=None)
	tb_3.columns = ['gene']
	tb = pd.merge(tb_1, tb_2, left_on='SNP', right_on='SNP')
	tb = tb.loc[tb.groupby('gene')['P'].idxmin()]
	tb = pd.merge(tb_3, tb, left_on='gene', right_on='gene', how='left')
	tmp = tb[['gene', 'P']]	
	tmp = tmp.fillna(0)
	tmp.to_csv(out_f, index=False, header=None, sep=' ')
...
#gsmr_file(b_pre='data/arabi', exp_f='data/GSE54680_10C_quant.plink.exp', out_d='data/gsmr', test=False)
def chr():
...
def mateqtl_input(b_pre='test/test', exp='data/GSE54680_10C_raw_quantile.txt', out_pre='test/test_2'):
	if not out_pre:
		out_pre = b_pre
	fam_f = b_pre + '.fam'
	exp_f = out_pre + '.exp'
...
#	gsmr_file(b_pre='data/arabi', exp_f='data/GSE54680_10C_quant.plink.exp', out_d='data/gsmr', test=False)
	mateqtl_input(b_pre='data/arabi', exp='data/GSE54680_10C_quant.csv', out_pre='data/mateqtl/arabi')
	pass	
